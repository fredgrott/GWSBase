...
public abstract class MaterialMenuBase implements MaterialMenu {
    ...
    /**
     * Overwrites behaviour of pressed state circle animation even when using #animatePressedState(MaterialMenuDrawable.IconState)}
     *
    ...
    public void syncState(Bundle state) {
        if (state != null) {
            ...
            if (iconStateName == null) {
                iconStateName = IconState.BURGER.name();
            }
