...
public class SVGParser extends DefaultHandler2
{
       ...
       */
        ...
        public SVG.Unit nextUnit()
        {
            ...
            if (position > (inputLength - 2))
                return null;
            try {
                SVG.Unit result = SVG.Unit.valueOf(input.substring(position, position + 2).toLowerCase(Locale.US));
                position +=2;
                return result;
            } catch (IllegalArgumentException e) {
                return null;
    ...
    */
    private static List<SVG.Length>  parseLengthList(String val) throws SAXException
    {
        ...
        List<SVG.Length>  coords = new ArrayList<SVG.Length>(1);
    ...
    */
    private static SVG.Colour parseColour(String val) throws SAXException
    {
        ...
        if (val.toLowerCase(Locale.US).startsWith("rgb("))
        {
            ...
            return new SVG.Colour(clamp255(red)<<16 | clamp255(green)<<8 | clamp255(blue));
        }
    ...
    private static SVG.Colour parseColourKeyword(String name) throws SAXException
    {
        ...
        return new SVG.Colour(col.intValue());
    }
    ...
    private static SVG.Length[]  parseStrokeDashArray(String val) throws SAXException
    {
        ...
        List<SVG.Length> dashes = new ArrayList<SVG.Length>();
        dashes.add(dash);
        while (!scan.empty())
        {
            scan.skipCommaWhitespace();
